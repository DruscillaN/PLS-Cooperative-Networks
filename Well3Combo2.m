...
% Numerical Integration Settings (ZFB Scheme 1)
y_max   = 1e4;
rel_tol = 1e-8;
abs_tol = 1e-10;
...
% Markers and linestyles: solid for ZFB Scheme 1, dashed for NAN
markers_s1 = {'-s', '-o', '-^', '-d'};
markers_s2 = {'--s', '--o', '--^'};
...
for m_idx = 1:length(M_scheme1)
    ...
    allSOP = [allSOP, SOP];
    semilogy(SNR_dB, SOP, markers_s1{m_idx}, 'LineWidth', 2, 'MarkerSize', 6, ...
        'MarkerFaceColor', colors_s1(m_idx,:), 'Color', colors_s1(m_idx,:));
    legendentries{end+1} = sprintf('Scheme 1, M = %d', M);
end
...
for m_idx = 1:length(M_scheme2)
    ...
    allSOP = [allSOP, SOP];
    semilogy(SNR_dB, SOP, markers_s2{m_idx}, 'LineWidth', 2, 'MarkerSize', 6, ...
...
title(['Combined SOP vs SNR (R_s = ', num2str(Rs), ' bits/s/Hz)'], 'FontSize', 13);
legend(legendentries, 'Location', 'SouthWest', 'FontSize', 10, 'NumColumns', 2);

% Force powers-of-10 on Y-axis
ax = gca;
