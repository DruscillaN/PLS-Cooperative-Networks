...
for m_idx = 1:length(M_scheme1)
    M   = M_scheme1(m_idx);
    SOP = zeros(size(SNR_RE_lin));
    fprintf('Computing Scheme 1 SOP for M = %d...\n', M);

    for idx = 1:length(SNR_RE_lin)
        % g_RE is the sweep variable; derive all others via Omega ratios
        g_RE = SNR_RE_lin(idx);
        g_AR = g_RE * (Omega_AR / Omega_RE);
        g_RB = g_RE * (Omega_RB / Omega_RE);
        g_AE = g_RE * (Omega_AE / Omega_RE);
        ...
        SOP(idx) = T1 + T2 + T3 + T4;
        fprintf('SNR_RE=%-6.1f dB | T1=%-10.4f T2=%-10.4f T3=%-10.4f T4=%-10.4f SOP=%-12.4e\n', ...
                SNR_RE_dB(idx), T1, T2, T3, T4, SOP(idx));
    end

    allSOP = [allSOP, SOP];
    semilogy(SNR_RE_dB, SOP, markers_s1{m_idx}, 'LineWidth', 2, 'MarkerSize', 6, ...
...
for m_idx = 1:length(M_scheme2)
    ...
    allSOP = [allSOP, SOP];
    semilogy(SNR_RE_dB, SOP, markers_s2{m_idx}, 'LineWidth', 2, 'MarkerSize', 6, ...
...
% =========================================================
grid on; grid minor;
xlabel('Average SNR_{RE} (dB)', 'FontSize', 12);
ylabel('Secrecy Outage Probability (SOP)', 'FontSize', 12);
title(['SOP vs SNR_{RE}  (R_s = ', num2str(Rs), ' bits/s/Hz)'], 'FontSize', 13);
