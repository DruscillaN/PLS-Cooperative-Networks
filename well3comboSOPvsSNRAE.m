...
% M values per scheme
M_scheme1 = [2, 3, 5];   % ZFB scheme
M_scheme2 = [2, 3, 5];   % NAN scheme
...
for m_idx = 1:length(M_scheme1)
    M   = M_scheme1(m_idx);
    SOP = zeros(size(SNR_AE_lin));
    fprintf('Computing Scheme 1 SOP for M = %d...\n', M);

    for idx = 1:length(SNR_AE_lin)
        % g_AE is the sweep variable; derive all others via Omega ratios
        g_AE = SNR_AE_lin(idx);
        g_AR = g_AE * (Omega_AR / Omega_AE);
        g_RB = g_AE * (Omega_RB / Omega_AE);
        g_RE = g_AE * (Omega_RE / Omega_AE);
        ...
        SOP(idx) = T1 + T2 + T3 + T4;
        fprintf('SNR_AE=%-6.1f dB | T1=%-10.4f T2=%-10.4f T3=%-10.4f T4=%-10.4f SOP=%-12.4e\n', ...
                SNR_AE_dB(idx), T1, T2, T3, T4, SOP(idx));
    end

    allSOP = [allSOP, SOP];
    semilogy(SNR_AE_dB, SOP, markers_s1{m_idx}, 'LineWidth', 2, 'MarkerSize', 6, ...
...
for m_idx = 1:length(M_scheme2)
    ...
    allSOP = [allSOP, SOP];
    semilogy(SNR_AE_dB, SOP, markers_s2{m_idx}, 'LineWidth', 2, 'MarkerSize', 6, ...
...
% =========================================================
grid on; grid minor;
xlabel('Average SNR_{AE} (dB)', 'FontSize', 12);
ylabel('Secrecy Outage Probability (SOP)', 'FontSize', 12);
title(['SOP vs SNR_{AE}  (R_s = ', num2str(Rs), ' bits/s/Hz)'], 'FontSize', 13);
